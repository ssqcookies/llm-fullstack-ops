    ...
    def _validate_draft_app_config(self, draft_app_config: dict[str, Any], account: Account) -> dict[str, Any]:
        ...
        if "tools" in draft_app_config:
            ...
            if not isinstance(tools, list):
                raise ValidationException("工具列表必须是列表型数据")
            # 6.2 tools的长度不能超过5
            if len(tools) > 5:
                raise ValidationException("Agent绑定的工具数不能超过5")
            # 6.3 循环校验工具里的每一个参数
            for tool in tools:
                # 6.4 校验tool非空并且类型为字典
                if not tool or not isinstance(tool, dict):
                    raise ValidationException("绑定插件工具参数出错")
                ...
                if (
                    ...
                    raise ValidationException("插件提供者或者插件标识参数出错")
                # 6.8 校验params参数，类型为字典
                if not isinstance(tool["params"], dict):
                    raise ValidationException("插件自定义参数格式错误")
            ...
            if len(set(check_tools)) != len(validate_tools):
                raise ValidationException("绑定插件存在重复")
        ...
        if "datasets" in draft_app_config:
            ...
            if not isinstance(datasets, list):
                raise ValidationException("绑定知识库列表参数格式错误")
            # 8.2 判断关联的知识库列表是否超过5个
            if len(datasets) > 5:
                raise ValidationException("Agent绑定的知识库数量不能超过5个")
            # 8.3 循环校验知识库的每个参数
            for dataset_id in datasets:
                try:
                    UUID(dataset_id)
                except Exception as e:
                    raise ValidationException("知识库列表参数必须是UUID")
            # 8.4 判断是否传递了重复的知识库
            if len(set(datasets)) != len(datasets):
                raise ValidationException("绑定知识库存在重复")
        ...
        if "retrieval_config" in draft_app_config:
            ...
            if not retrieval_config or not isinstance(retrieval_config, dict):
                raise ValidationException("检索配置格式错误")
            # 9.2 校验检索配置的字段类型
        ...
        if "opening_statement" in draft_app_config:
            ...
            if not isinstance(opening_statement, str) or len(opening_statement) > 2000:
                raise ValidationException("对话开场白的长度范围是0-2000")
        ...
        if "speech_to_text" in draft_app_config:
            ...
            if not speech_to_text or not isinstance(speech_to_text, dict):
                raise ValidationException("语音转文本设置格式错误")
            # 13.2 校验语音转文本属性
            if (
        ...
        if "review_config" in draft_app_config:
            ...
            if (
                ...
                raise ValidationException("review.keywords非空且不能超过100个关键词")
            for keyword in review_config["keywords"]:
                if not isinstance(keyword, str):
                    raise ValidationException("review.keywords敏感词必须是字符串")
            ...
            if (
                ...
                raise ValidationException("review.inputs_config必须是一个字典")
            ...
            if (
                ...
                raise ValidationException("review.outputs_config格式错误")
            ...
            if review_config["enable"]:
                if (
                    ...
                    raise ValidationException("输入审核和输出审核至少需要开启一项")
