...
class WorkflowConfig(BaseModel):
    ...
    def validate_workflow_config(cls, values: dict[str, Any]):
        ...
        if not name or not re.match(WORKFLOW_CONFIG_NAME_PATTERN, name):
            raise ValidationException("工作流名字仅支持字母、数字和下划线，且以字母/下划线为开头")
        ...
        if not description or len(description) > WORKFLOW_CONFIG_DESCRIPTION_MAX_LENGTH:
            raise ValidationException("工作流描述信息长度不能超过1024个字符")
        ...
        if not isinstance(nodes, list) or len(nodes) <= 0:
            raise ValidationException("工作流节点列表信息错误，请核实后重试")
        if not isinstance(edges, list) or len(edges) <= 0:
            raise ValidationException("工作流边列表信息错误，请核实后重试")
        ...
        if (
            ...
            raise ValidationException("工作流中有且只有一个开始/结束节点作为图结构的起点和终点")
        ...
        if not cls._is_connected(adj_list, start_node_data.id):
            raise ValidationException("工作流中存在不可到达节点，图不联通，请核实后重试")

        # 24.校验edges中是否存在环路（即循环边结构）
        if cls._is_cycle(node_data_dict.values(), adj_list, in_degree):
            raise ValidationException("工作流中存在环路，请核实后重试")
                ...
                for variable in variables:
                    ...
                    if variable.value.type == VariableValueType.REF:
                        ...
                        if (
                            ...
                            raise ValidationException(f"工作流节点[{node_data.title}]引用数据出错，请核实后重试")
