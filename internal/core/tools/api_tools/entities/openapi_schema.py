...
class OpenAPISchema(BaseModel):
    ...
    def validate_paths(cls, paths: dict[str, dict]) -> dict[str, dict]:
        ...
        if not paths or not isinstance(paths, dict):
            raise ValidationException("openapi_schema中的paths不能为空且必须为字典")
        ...
        for interface in interfaces:
            ...
            if interface["operation"]["operationId"] in operation_ids:
                raise ValidationException(f"operationId必须唯一，{interface['operation']['operationId']}出现重复")
            ...
            for parameter in interface["operation"].get("parameters", []):
                ...
                if not isinstance(parameter.get("name"), str):
                    raise ValidationException("parameter.name参数必须为字符串且不为空")
                if not isinstance(parameter.get("description"), str):
                    raise ValidationException("parameter.description参数必须为字符串且不为空")
                if not isinstance(parameter.get("required"), bool):
                    raise ValidationException("parameter.required参数必须为布尔值且不为空")
                if (
